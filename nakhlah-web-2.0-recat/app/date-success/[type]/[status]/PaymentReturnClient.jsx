...
export default function PaymentReturnClient() {
  ...
  const [captureState, setCaptureState] = useState({
    ...
    message: isCanceled
      ? "Your checkout was canceled, so no charge was completed."
  ...
  useEffect(() => {
    ...
    const capturePayment = async () => {
      if (!orderId) {
        setCaptureState({
          status: "error",
          message: "Missing order token. Please contact support.",
        });
        return;
      }
      ...
      if (!result.success) {
        ...
        setCaptureState({
          status: "error",
          message: result.error || "Unable to confirm your payment.",
        });
        toast.error(result.error || "Unable to confirm payment.");
        return;
      }
