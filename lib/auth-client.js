  ...
  const scheduleTokenRefresh = useCallback(
    (exp, delay = null) => {
      ...
      const msUntilRefresh =
        delay ?? (exp ? exp * 1000 - Date.now() - 60_000 : REFRESH_RETRY_MS);
      ...
      refreshTimerRef.current = setTimeout(async () => {
        ...
        scheduleRef.current?.(current.exp, REFRESH_RETRY_MS);
    ...
    if (stored) {
      ...
      if (stored.exp && now > stored.exp) {
        (async () => {
          ...
          setSessionData(stored);
          scheduleTokenRefresh(stored.exp, REFRESH_RETRY_MS);
        })();
      } else {
        queueMicrotask(() => {
          setSessionData(stored);
  ...
  if (provider === "google") {
    ...
    if (isCapacitor) {
      try {
        ...
        const googleToken = result.accessToken || result.idToken;
        if (!googleToken || !result.email || !result.userId) {
          return {
            ok: false,
            error: "Google sign-in did not return a usable account token",
          };
        }
