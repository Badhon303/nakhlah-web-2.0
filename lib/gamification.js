...
export const PALM_REFILL_INTERVAL_MS = 60 * 60 * 1000; // 1 hour per Palm Tree
export const DEFAULT_MAX_PALM_STOCK = 5;

/**
 * Computes when the next Palm Tree will refill given the server-provided
 * `palmUpdatedAt` timestamp (converted to local time via the Date object)
 * and the free refill interval (1 hour per Palm Tree).
 */
export function getPalmRefillState(
  palmUpdatedAt,
  palmStock,
  maxPalmStock = DEFAULT_MAX_PALM_STOCK,
) {
  const stock = Number(palmStock);
  const isFull = Number.isFinite(stock) && stock >= maxPalmStock;

  if (isFull || !palmUpdatedAt) {
    return { isFull: true, nextRefillAt: null, msRemaining: 0 };
  }

  const updatedAtMs = new Date(palmUpdatedAt).getTime();
  if (!Number.isFinite(updatedAtMs)) {
    return { isFull: true, nextRefillAt: null, msRemaining: 0 };
  }

  const nextRefillAtMs = updatedAtMs + PALM_REFILL_INTERVAL_MS;
  const msRemaining = Math.max(0, nextRefillAtMs - Date.now());

  return {
    isFull: msRemaining <= 0,
    nextRefillAt: new Date(nextRefillAtMs),
    msRemaining,
  };
}

/**
 * Formats a countdown duration as "Xh Ym" when over an hour, otherwise
 * "MM:SS".
 */
export function formatRefillCountdown(msRemaining) {
